...
RSpec.describe Hanami::CygUtils::PathPrefix do
  it "exposes itself as a string" do
    prefix = Hanami::CygUtils::PathPrefix.new
    expect(prefix).to eq("")
  end

  it "adds root prefix only when needed" do
    prefix = Hanami::CygUtils::PathPrefix.new("/fruits")
    expect(prefix).to eq("/fruits")
  end

  describe "#join" do
    it "returns a PathPrefix" do
      prefix = Hanami::CygUtils::PathPrefix.new("orders", "?").join("new")
      expect(prefix).to be_kind_of(Hanami::CygUtils::PathPrefix)
    ...
    it "joins multiple strings" do
      ...
      prefix = Hanami::CygUtils::PathPrefix.new("myapp")
    ...
    it "rejects entries that are matching separator" do
      prefix = Hanami::CygUtils::PathPrefix.new("/assets")
      expect(prefix.join("/")).to eq "/assets"
    end

    it "removes trailing occurrences of separator" do
      prefix = Hanami::CygUtils::PathPrefix.new("curcuma")
  ...
  describe "#relative_join" do
    it "returns a PathPrefix" do
      prefix = Hanami::CygUtils::PathPrefix.new("orders", "&").relative_join("new")
      expect(prefix).to be_kind_of(Hanami::CygUtils::PathPrefix)
      expect(prefix.__send__(:separator)).to eq("&")
    end
    ...
    it "removes trailing occurrences of separator" do
      prefix = Hanami::CygUtils::PathPrefix.new("jojoba")
  ...
  describe "string interface" do
    describe "equality" do
      it "has a working equality" do
        string = Hanami::CygUtils::PathPrefix.new("hanami")
        other  = Hanami::CygUtils::PathPrefix.new("hanami")
    ...
    describe "#hash" do
      it "returns the same hash result of ::String" do
        expected = "hello".hash
        actual   = Hanami::CygUtils::PathPrefix.new("hello").hash
